...
/*   Updated: 2019/06/14 17:06:03 by maegaspa    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int        ox_treat_5(t_flag flag, long long dig, char *nbr, int nb_char)
{
    ...
    if (flag.point && flag.precision)
	{
		...
		if (dig == 0 && flag.hashtag && flag.point && flag.precision && !flag.width)
...
int            ox_treat_6(t_flag flag, long long dig, char *nbr, int nb_char)
{
    ...
    putspace = 0;
    i = -1;
    if (flag.hashtag > 0 && (!(dig == 0)) && (!flag.width && !flag.precision && !flag.zero))
    {
        if (flag.conv == 'x')
...
int         ox_treat_7(t_flag flag, long long dig, char *nbr, int nb_char)
{
	if (dig == 0 && flag.point && !flag.width)
		nbr[0] = '\0';
	else if (dig == 0 && !flag.width && !flag.precision && !flag.point && flag.hashtag)
		return (nb_char);
